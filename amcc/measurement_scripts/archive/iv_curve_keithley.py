...
def vi_curve(k, currents = [0,1e-3,2e-3], compliance_v = 1, step_delay = 0.05, plc_value = 1.0):
...
voltages = np.linspace(-0.6, 0.6, 501); compliance_i = 500e-6
# voltages = np.concatenate([voltages, voltages[::-1]])
# voltages = np.concatenate([voltages, -voltages])
num_sweeps = 1
test_name = 'Niccolo Design - mdeveloped by Di - H1'
...
for n in range(num_sweeps):
    ...
    V, I = iv_curve(k, voltages = voltages, compliance_i = compliance_i, step_delay = 0.05, plc_value = 3.0)
    V_list.append(V)
    I_list.append(I)
    # plt.plot(V,I,'.-')
