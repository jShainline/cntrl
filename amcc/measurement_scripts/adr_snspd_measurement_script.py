...
T = 3
R_bias = 10e3
currents = np.linspace(0, 20e-6, 101)
counting_time = 0.5
trigger_voltage = 0.03
attenuations_db = [np.inf,0,15,30] # None == np.inf aka beam is blocked
ports = [1]
...
filename = datetime.datetime.now().strftime('%Y-%m-%d %H-%M-%S IV Curve ' + test_name + ' T=%0.1f K' % T)
...
# Save data
file_dir = 'C:\\Users\\qittlab\\Desktop\\Adam Python Data Folder\\'
filename = datetime.datetime.now().strftime('%Y-%m-%d %H-%M-%S Counts vs Bias' + test_name  + ' T=%0.1f K' % T)
df = pd.DataFrame(data)
df.to_csv(file_dir + filename + '.csv')
...
for port in ports:
    ...
    ax.legend()
plt.tight_layout()
fig.suptitle('Counts vs Bias @ T = %0.1f K' % (T)); fig.subplots_adjust(top=0.95) # Add supertitle over all subplots
